    ...
    I: CSTInput,
{
    ...
    fn parse(input: &mut I) -> Result<Self, <I as parserc::Input>::Error> {
        let content = take_while_range_from(1, |c: u8| c.is_ascii_digit())
            .parse(input)
            .map_err(SyntaxKind::LitDigits.map())?;

        let value = u128::from_str_radix(content.as_str(), 10).map_err(|_| {
            CSTError::Syntax(SyntaxKind::LitDigits, ControlFlow::Fatal, content.to_span())
        })?;
    ...
    I: CSTInput,
{
    if number.trunc.is_none() && number.fract.is_none() {
        let control_flow = if number.sign.is_some() || number.exp.is_some() {
            ControlFlow::Fatal
        } else {
            ControlFlow::Recovable
        };
